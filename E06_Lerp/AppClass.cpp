...
void AppClass::InitVariables(void)
{
	...
	m_pShpere = new PrimitiveClass[m_nObjects];
	...
	for (int i = 0; i < m_nObjects; i++) {
		...
		m_pShpere[i].GenerateSphere(2.0f, 5, vector3(fPercent, 0.0f, 0.0f));
		m_pMatrix[i] = glm::translate(fPercent * v3Start + (1 - fPercent) * v3End);
	}
...
void AppClass::Display(void)
{
	...
	for (int i = 0; i < m_nObjects; i++) {
		...
		m_pShpere[i].Render(m4Projection, m4View, m_pMatrix[i]);
	}
